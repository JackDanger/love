...
namespace love
{
	...
	const string & ImageFromFile::getFilename() const
	{
		return file->getFilename();
	}

	int ImageFromFile::twoPower(int num) const
	{
		int temp = 1;
		while(temp < num)
			temp *= 2;
	...
	void ImageFromFile::padTwoPower()
	{
	...
	{	
		...
		// This also unloads the data in LoveFile, so no need to do that.
		ilDeleteImages(1, &image);
