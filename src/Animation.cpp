...
namespace love
{
	...
	void Animation::addFrame(float x, float y, float w, float h, float delay)
	{
		...
		float xTex = x/(float)image->textureWidth;
		float yTex = y/(float)image->textureHeight;

		float wTex = w/(float)image->textureWidth;
		float hTex = h/(float)image->textureHeight;
	...
	void Animation::render() const
	{
		...
		image->render(f.vertices, f.texels, 0, 0, 0, 1, 1, -f.width/2.0f + center_x, -f.height/2.0f + center_y);
	}
	...
	void Animation::render(float x, float y) const
	{
		...
		const frame & f = frames[current];
		image->render(f.vertices, f.texels, x, y, 0, 1, 1, -f.width/2.0f + center_x, -f.height/2.0f + center_y);
	}
	...
	void Animation::render(float x, float y, float angle, float sx, float sy) const
	{
		...
		const frame & f = frames[current];
		image->render(f.vertices, f.texels, x, y, angle, sx, sy, -f.width/2.0f + center_x, -f.height/2.0f + center_y);
	}
