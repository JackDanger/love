...
class phpthumb {
	...
	var $phpthumb_version = '1.7.1-200603160840';
	...
	function GenerateThumbnail() {
		...
		$this->DebugMessage('ImageResizeFunction($o, $s, '.$destination_offset_x.', '.$destination_offset_y.', '.$this->thumbnailCropX.', '.$this->thumbnailCropY.', '.$this->thumbnail_image_width.', '.$this->thumbnail_image_height.', '.$this->thumbnailCropW.', '.$this->thumbnailCropH.')', __FILE__, __LINE__);
		...
		if ($this->useRawIMoutput) {
			...
			switch ($this->thumbnailFormat) {
				...
				case 'bmp':
					...
					if (!file_put_contents($renderfilename, $bmp_data)) {
						$this->DebugMessage('file_put_contents('.$renderfilename.', ['.strlen($bmp_data).' bytes]) failed', __FILE__, __LINE__);
						ob_end_clean();
						return false;
					}
					break;
				...
				case 'ico':
					...
					if (!file_put_contents($renderfilename, $ico_data)) {
						$this->DebugMessage('file_put_contents('.$renderfilename.', ['.strlen($ico_data).' bytes]) failed', __FILE__, __LINE__);
						ob_end_clean();
						return false;
					}
					break;
			...
			if ($dirhandle = opendir($this->config_cache_directory)) {
				while ($oldcachefile = readdir($dirhandle)) {
					if (eregi('^phpThumb_cache_', $oldcachefile)) {
						$CacheDirOldFilesAge[$oldcachefile] = fileatime($this->config_cache_directory.DIRECTORY_SEPARATOR.$oldcachefile);
						if ($CacheDirOldFilesAge[$oldcachefile] == 0) {
							$CacheDirOldFilesAge[$oldcachefile] = filemtime($this->config_cache_directory.DIRECTORY_SEPARATOR.$oldcachefile);
						}

						$CacheDirOldFilesSize[$oldcachefile] = filesize($this->config_cache_directory.DIRECTORY_SEPARATOR.$oldcachefile);
					}
				}
			}
			...
			if ($this->config_cache_maxfiles > 0) {
				...
				foreach ($CacheDirOldFilesAge as $oldcachefile => $filedate) {
					if ($TotalCachedFiles > $this->config_cache_maxfiles) {
						$TotalCachedFiles--;
						if (@unlink($this->config_cache_directory.DIRECTORY_SEPARATOR.$oldcachefile)) {
				...
				if ($getimagesize = GetImageSize($this->sourceFilename)) {
					...
					if ($this->source_width || $this->source_height) {
						if ($this->zc) {
							...
							$wAll = intval(max($this->w, $this->wp, $this->wl, $this->ws));
							$hAll = intval(max($this->h, $this->hp, $this->hl, $this->hs));
							$side  = min($this->source_width, $this->source_height, max($wAll, $hAll));
							$sideX = min($this->source_width,                       $wAll);
							$sideY = min(                     $this->source_height, $hAll);
							if ($getimagesize[0] > $getimagesize[1]) {
								$commandline .= ' -thumbnail x'.$sideY;
							} else {
								$commandline .= ' -thumbnail '.$sideX.'x';
							}
							...
							$this->w = (($this->aoe && $this->w) ? max($this->w, $getimagesize[0]) : ($this->w ? phpthumb_functions::nonempty_min($this->w, $getimagesize[0]) : ''));
							$this->h = (($this->aoe && $this->h) ? max($this->h, $getimagesize[1]) : ($this->h ? phpthumb_functions::nonempty_min($this->h, $getimagesize[1]) : ''));
							if ($this->w || $this->h) {
...
//phpthumb_functions::gd_ImageSaveAlpha($this->gdimg_source, true);
...
//exit;
	...
	function Rotate() {
		if ($this->ra || $this->ar) {
			...
			if ($rotate_angle != 0) {
				...
				//if ((phpthumb_functions::gd_version() >= 2) && in_array($this->thumbnailFormat, $this->AlphaCapableFormats) && !$this->bg && ($rotate_angle % 90)) {
	...
	function AlphaChannelFlatten() {
		...
		if ($gdimg_flatten_temp = phpthumb_functions::ImageCreateFunction($this->thumbnail_width, $this->thumbnail_height)) {
			...
			phpthumb_functions::gd_ImageSaveAlpha($this->gdimg_output, false);
							...
							if ($img_watermark = $this->ImageCreateFromStringReplacement($WatermarkImageData)) {
								...
								if ($underlay) {
									...
									if ($img_watermark_resized = phpthumb_functions::ImageCreateFunction(ImageSX($this->gdimg_output), ImageSY($this->gdimg_output))) {
										...
										phpthumb_functions::gd_ImageSaveAlpha($img_watermark_resized, true);
										...
										if ($img_source_resized = phpthumb_functions::ImageCreateFunction($resized_x, $resized_y)) {
											...
											phpthumb_functions::gd_ImageSaveAlpha($img_source_resized, true);
	...
	function CreateGDoutput() {
		...
		phpthumb_functions::gd_ImageSaveAlpha($this->gdimg_output, true);
	...
	function SetCacheFilename() {
		...
		if ($this->config_cache_default_only_suffix && $this->sourceFilename) {
			...
			if (strpos($this->config_cache_default_only_suffix, '*') === false) {
				...
				$this->cache_filename = $this->config_cache_directory.DIRECTORY_SEPARATOR.str_replace('*', $matches[1], $this->config_cache_default_only_suffix).'.'.strtolower($this->thumbnailFormat);
				return true;
			}
		}

		$this->cache_filename = $this->config_cache_directory.DIRECTORY_SEPARATOR.$this->config_cache_prefix;
		if ($this->new) {
	...
	function ImageCreateFromFilename($filename, &$rawimagedata) {
		...
		$this->DebugMessage('starting ImageCreateFromFilename('.$filename.', ['.strlen($rawimagedata).' bytes])', __FILE__, __LINE__);

		if ($filename && ($getimagesizeinfo = @GetImageSize($filename))) {
			if (($this->config_max_source_pixels > 0) && (($getimagesizeinfo[0] * $getimagesizeinfo[1]) > $this->config_max_source_pixels)) {
	...
	function phpThumb_tempnam() {
		if (!$this->config_temp_directory) {
			$this->config_temp_directory = (getenv('TMPDIR') ? getenv('TMPDIR') : getenv('TMP'));
		}
